...
**iptables** Linux上的防火墙配置工具就无需过多介绍了吧，前期用来配置路由就是靠它，接下来在科学上网中它也将担任重要的功能

## 原理

---

> 本例中使用了V2Ray自身支持的一个功能协议，名字是 **[dokodemo-door](https://v2ray.com/chapter_02/protocols/dokodemo.html)** 官方的说明文档把每个参数都有说明，如果配置成功的话，基本上透明代理的效果会很好（前提你自己有一个比较高速的梯子，或者机场也行，最近支持V2Ray的机场好像变多了），此处我已经将这个入口配置好
>
> 当然对于很多人来说这样的说明真的会一头雾水，让人无从下手，还好官网直接提供了两个白话文教程，[白话文教程](https://toutyrater.github.io/app/transparent_proxy.html)、[新白话文教程（社区版）](https://guide.v2fly.org/app/transparent_proxy.html#%E8%AE%BE%E7%BD%AE%E6%AD%A5%E9%AA%A4)，实际上呢，内容差不多，任意一个都可以参考

那它是怎么实现科学上网的呢？图不高兴画了，就直接用文字来表述一下吧：

### 发起访问

假设你想科学上网，你会对路由器发起一次通讯，假设发送了一个数据包，这个数据包包含了来源地址，域名信息或者目标IP等等内容；

### 数据接收部分

到达路由器之后，首先会被**iptables**（实际上并不是它，而是内核中的[Netfilter](https://zh.wikipedia.org/wiki/Netfilter)，此处只是为了方便说明而已，使用iptables这个防火墙配置工具代指Linux系统的防火墙部分）接收到，此时iptables就会对这个数据包进行分析；
...
此时V2ray接收到了数据，它内置了一套路由系统，也会对数据包进行分析，如果数据包目标地址是国内的地址，那V2Ray不会对数据包进行处理，直接就按照原来的目标地址发送过去，而如果是国外的地址，就会通过预先配置好的梯子通道转发出去，这样就实现了科学上网，当然V2Ray的功能不止于此，迫于篇幅，本篇就不提及了。
...
V2Ray项目直接提供了官方的一键安装脚本，由于是使用GO语言编写的，除非自己喜欢折腾，基本上都不需要进行编译安装，一键脚本会自动从github上下载最新的编译好的二进制版本进行安装，同时它也会自动配置好自动启动，由于国内访问github的速度不理想，所以我在第一步的时候就配置好了代理以提高一键脚本的执行速度

执行完成之后会自动随机生成端口和UUID，并配置好自动启动，此处使用的是debian，如果是其它发行版的话，建议使用systemd版本的，一键脚本对于init的自动启动配置支持的并不好

### 3.V2Ray配置文件说明

V2Ray的默认配置文件是`/etc/v2ray/config.json`，如果使用的是一键脚本的话，默认安装的是服务端配置，此处我们打算建立一个本地的科学上网软路由，那么就需要将其修改为客户端模式，不过在这之前首先要对V2Ray的配置文件格式做一下简单介绍，打开配置文件`vi /etc/v2ray/config.json`，如下图所示：
...
根据官方教程，整个配置文件应该由如下几个部分构成，但这些部分都是可以经济的，但有几个部分一定是必须的，如果是服务端的话，**inbounds**、**outbounds**、**routing**这三个部分是必要的；而如果是客户端的话，一般会多一个**dns**模块提供域名解析的支持，当然如果你对配置文件比较熟悉的话，可以通过增加更多的模块实现更多的功能

**inbounds**这个部分作为主机的接受端，会监听进入系统的一些连接，一般情况下对外监听端口的都是对外提供服务的，此处也不例外，不管是服务端还是客户端，都是在**inbounds**处提供服务

**outbounds**进行对外连接，如果是在服务端，那么就是客户端流量的访问出口，同时也能对一些流量进行屏蔽，而在客户端，主要会提供和服务端之间进行对接的服务
...
> V2Ray对外连接的配置主要集中在**outbounds**部分，此处集中了梯子的连接，可以是一个或者多个，默认情况下如果没有在**routing**部分进行指定，系统都会使用由上至下第一个对外配置作为默认出口使用，所以排在第一个的具有举足轻重的地位，以下将会放上我自己的配置：
...
此处直接使用了最简单的配置，使用这样的配置的话，当用户使用socks5代理进行上网时，会优先使用谷歌和IBM的服务器进行域名解析，如果上述的服务器都挂了，那么就使用系统自带的DNS服务器进行解析

#### 强大的路由功能

> 一般的科学上网工具除了提供分流外等附加功能外基本上不会再有别的功能了，而V2Ray可通过**routing**这个功能模块提供类似于“Linux防火墙”一样的功能，根据配置好的规则选择对外出口，负载均衡，识别连接，屏蔽广告，基于端口匹配等等，当然这些功能对于初学者而言简直就是天书，本次也不会详细去说明，但大家只需要知道一点，那就是善用**routing**模块
...
以上的配置屏蔽了一些常用的广告域名，同时不代理内网IP、中国区IP、中国区域名、常见的没被墙的域名，其它的流量全部走默认的科学上网出口；我使用了配置Linux防火墙类似的配置顺序，需要屏蔽的放在上面，之后的规则会依次匹配，V2Ray的规则匹配也是这样的顺序，由上至下，哪个在上面哪个先匹配，一旦匹配到就跳出规则，所以在配置路由的时候需要好好规划好顺序
...
在上述的配置文件中我增加了一个log模块，主要是为了进行故障分析，在`/var/log/v2ray`目录下生成两个日志文件，方便遇到问题之后进行分析，整个配置文件除了科学上网部分没有配置正确，其它都能实际使用，这个部分是可以当作模板进行使用的
